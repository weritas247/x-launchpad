# Control Server — Super Terminal 프로세스 관리 대시보드
...
Super Terminal(포트 3000)을 켜고 끌 수 있는 별도 경량 컨트롤 서버(포트 3001)를 만든다. 컨트롤 서버는 항상 실행되며, Super Terminal 프로세스를 spawn/kill하고 상태를 모니터링한다.

서버가 꺼져 있을 때도 `localhost:3000`에 접근하면 "꺼짐 상태" 페이지를 보여주고, 켜져 있을 때는 Super Terminal 메인 UI에서 플로팅 버튼으로 컨트롤 패널에 접근할 수 있다.
...
│  ├─ child_process.spawn → Super Terminal (포트 3000) │
...
사용자 접근:
  브라우저 → localhost:3000
    ├─ 서버 ON  → Super Terminal UI (+ 왼쪽 아래 플로팅 버튼)
    └─ 서버 OFF → 컨트롤 서버가 3000에 바인딩 → "꺼짐 상태" UI
```

### 포트 전환 메커니즘

1. **Super Terminal OFF**: 컨트롤 서버가 포트 3000에 미니 HTTP 서버를 바인딩하여 "꺼짐 상태" 페이지 제공
2. **POST /api/start**: 포트 3000 미니 서버를 close → Super Terminal을 `child_process.spawn`으로 시작 (포트 3000 사용)
3. **POST /api/stop**: Super Terminal 프로세스를 graceful kill → 포트 3000에 미니 서버 다시 바인딩

### Readiness Detection (시작 완료 감지)

포트 3000 release 후 Super Terminal이 실제로 바인딩할 때까지 타이밍 갭이 존재한다. 이를 처리하는 흐름:

1. 포트 3000 미니 서버 `server.close()` 콜백 수신
2. Super Terminal spawn
...
POST /api/start      → Super Terminal 시작
POST /api/stop       → Super Terminal 종료 (graceful)
...
- LAN 접근이 필요한 경우: 환경변수 `CONTROL_HOST=0.0.0.0` 설정 시 기존 Super Terminal JWT 인증 재사용
...
- **sessions**: Super Terminal의 `/api/sessions` 호출 또는 WebSocket으로 조회
...
- Super Terminal 로고 + "서버가 꺼져 있습니다" 메시지
...
### 2. 플로팅 버튼 (Super Terminal 메인 UI, 왼쪽 아래)

Super Terminal이 켜져 있을 때 메인 UI에 표시되는 원형 버튼.
...
client/js/
└── control-panel.js      # 플로팅 버튼 + 패널 (Super Terminal 내)
```
...
`dev:all`은 컨트롤 서버만 시작. Super Terminal은 컨트롤 서버가 관리.

## Integration with Super Terminal
...
- Super Terminal 서버 코드(`server/index.ts`) 변경 없음
...
6. 컨트롤 서버: 포트 3000 release → Super Terminal spawn
7. Super Terminal 시작 완료 → 브라우저 자동 리다이렉트 → Super Terminal UI
...
AUTO_START=0               # 1이면 컨트롤 서버 시작 시 Super Terminal 자동 시작
```

## Edge Cases

- **Super Terminal 크래시**: exit 이벤트 감지 → 포트 3000 다시 바인딩 → 꺼짐 페이지에 "비정상 종료" 표시
- **포트 3000 이미 사용 중**: 시작 시 포트 충돌 감지 → 에러 메시지
- **컨트롤 서버 자체 종료**: 컨트롤 서버 종료 시 Super Terminal도 같이 종료할지 선택 가능 (설정)
